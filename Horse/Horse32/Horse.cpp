...
static BOOL HorseInit(HINSTANCE);
static void DrawBitmap(HDC hDC, short xStart, short yStart, HBITMAP hBitmap);
static BOOL FAR PASCAL About(HWND, unsigned, WPARAM, LPARAM);
static void DrawHorse(HWND hWnd,int swtch);

LRESULT CALLBACK HorseWndProc(HWND, UINT, WPARAM, LPARAM);
...
/* 	    	   	    current instance   previous instance     command line   show-window type (open/icon) */
{
	...
	int	wX,wY;
		...
		HANDLE hSema = ::CreateSemaphore(NULL, 0, 100, APPNAME);
		::ReleaseSemaphore(hSema, 1, &InstNum);
	...
	if ( InstNum == 0 &&
	     (GetProfileString(_T("Galloping Horse"), _T("Position"), _T(""), szSTR, sizeof(szSTR))))
	{
		PTCHAR	context;
		wX = _ttoi(_tcstok_s(szSTR, _T(","), &context));
		wY = _ttoi(_tcstok_s(NULL, _T(","), &context));
	}
	else
	{
		wX = CW_USEDEFAULT;
	...
	if (InstNum==0)
		_tcscpy_s(szSTR, sizeof(szSTR)/sizeof(szSTR[0]),_T("Horse"));
	else
		wsprintf(szSTR,_T("Horse %d"),InstNum+1);
	...
	AppendMenu(hlMenu,MF_STRING,IDM_ABOUT,_T("&About..."));
...
static BOOL HorseInit(HINSTANCE  hInstance)
         	   /* current instance  */
{
...
LRESULT  CALLBACK HorseWndProc(HWND hWnd,UINT message, WPARAM wParam, LPARAM lParam)
/* 			     window handle  type of message      additional information  */
{
	...
	switch (message)
	{
	   ...
	   case WM_SIZE:
		...
		if (InstNum==0) 
		{
			wsprintf(szPosition,_T("%d,%d"),LOWORD(lParam),
				HIWORD(lParam)-GetSystemMetrics(SM_CYCAPTION));
			WriteProfileString(_T("Galloping Horse"), _T("Position"),szPosition);
			}
		break;
            ...
            case WM_SYSCOMMAND: 
				if (wParam==IDM_ABOUT)
				{
					...
					DialogBox(GetInstance(hWnd), _T("AboutBox"),  hWnd, lpProcAbout);
...
BOOL FAR PASCAL About(HWND hDlg, unsigned int message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
	{
	case WM_INITDIALOG:
